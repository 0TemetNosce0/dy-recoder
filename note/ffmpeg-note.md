...
AVStream AVCodecContext AVCodec AVCodecParameters
...
AVCodec AVCodecContext AVCodecParameters AVCodecDescriptor
...
```
AVCodec *avcodec_find_decoder(enum AVCodecID id) //创建解码器,通过 id
AVCodec *avcodec_find_decoder_by_name(const char *name) //创建解码器,通过名字
//AVCodecID 是枚举值(eg:AV_CODEC_ID_H264),const char *name ffmpeg中唯一(eg:aac).

AVCodecContext * avcode_alloc_context3(const AVCodec *codec)//初始化解码器上下文,AVCodecContext需要一个AVCodec传入
void avcodec_free_context(AVCodecContext ** avcodecContext)//释放解码器上下文
int avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **option)//打开解码器

int avcodec_parameters_to_context(AVCodecContext *codec,const AVCodecParameters *par);//将stream流中的 codec的parameters 拷贝到AVCodecContext中，AVStream中弃用了 AVCodecde的替换方法
//////////////////////
...
```
//AVCodecContext 结构题的成员还有很多，这里只是摘取了常见的做了说明
...
AVFormatContext包含了一个媒体流的解码器的上下文信息，比较重要的有：
...
## AVCodecParameters  AVCodecDescriptor
